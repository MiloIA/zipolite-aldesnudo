...
export async function crearReservaYPago(token, chatId, estado) {
  ...
  const montoAnticipo = estado.tipo_pago === 'total'
    ? estado.precio * personas
    : estado.anticipo * personas;
  ...
  try {
    const { data: resData, error: resError } = await supabase
      ...
      .select()
      .single();
    console.log('INSERT RESULT:', JSON.stringify(resData));
    console.log('INSERT ERROR:', JSON.stringify(resError));
  ...
  if (metodo === 'transfer') {
    try {
      ...
      await sendMessage(token, chatId,
        ...
        `🔗 *Ver mi reserva:* https://zipolitealdesnudo.com/mi-reserva?id=${reserva.id}\n` +
        `👤 *Crear cuenta:* https://zipolitealdesnudo.com/mi-cuenta`
      );
    } catch (e) {
