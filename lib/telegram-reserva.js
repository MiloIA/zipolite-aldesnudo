...
export async function crearReservaYPago(token, chatId, estado) {
  ...
  if (metodo === 'transfer') {
    const { data: config } = await supabase
      .from('configuracion')
      .select('banco_nombre, banco_clabe, banco_beneficiario')
      .maybeSingle();
    const banco = config?.banco_nombre || 'BBVA';
    const clabe = config?.banco_clabe || '—';
    const beneficiario = config?.banco_beneficiario || 'Zipolite al Desnudo';
