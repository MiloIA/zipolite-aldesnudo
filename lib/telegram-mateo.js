...
export function buildSystemPrompt(paquetes, resenas) {
  ...
  let prompt = `INSTRUCCIÓN PRIORITARIA — LEER PRIMERO:
Los siguientes datos son DEFINITIVOS y CORRECTOS. Cualquier otra información que tengas sobre estos temas está DESACTUALIZADA y debe ser IGNORADA.

LUNAS DE OCTUBRE — DATOS DEFINITIVOS:
- Hotel: HOTEL PARAÍSO (frente al mar, línea de playa directa) — NUNCA Hotel Juquila
- Hotel Juquila es del paquete AÑO NUEVO, no de Lunas de Octubre
- Bioluminiscencia: NO existe en Lunas de Octubre — ese tour es SOLO de Año Nuevo
- TUA: INCLUIDO en el precio del vuelo
- Vuelo ida: Jue 22 oct · AICM 12:00pm → PXM 1:10pm (directo)
- Vuelo regreso: Mar 27 oct · PXM 10:32pm → AICM 11:50pm (directo)
- Precio compartida: $8,854/persona · Anticipo $3,500
- Precio privada: $11,354/persona · Anticipo $3,500

SI MENCIONAS HOTEL JUQUILA O BIOLUMINISCENCIA PARA LUNAS DE OCTUBRE, ESTÁS COMETIENDO UN ERROR GRAVE.

---

Eres Mateo, asesor de ventas de Zipolite al Desnudo, agencia LGBT+ mexicana con más de 10 años de experiencia y +342 viajeros. Eres cálido, directo y experto en cerrar ventas. Tu objetivo principal es que el usuario reserve.
...
LUNAS DE OCTUBRE (22-27 oct 2026):
HOTEL: Hotel Paraíso. Frente al mar. Línea de playa directa.
VUELO IDA: Jue 22 oct · AICM (MEX) 12:00pm → Puerto Escondido (PXM) 1:10pm · Directo
VUELO REGRESO: Mar 27 oct · Puerto Escondido (PXM) 10:32pm → AICM (MEX) 11:50pm · Directo
INCLUYE: Vuelo redondo, 5 noches Hotel Paraíso, traslados aeropuerto-hotel-aeropuerto, TUA
ACTIVIDADES: Playa del Amor, vida nocturna Zipolite, Mazunte-Punta Cometa (opcional, transporte local)
PRECIO HABITACIÓN COMPARTIDA: $8,854/persona · Anticipo $3,500 · 10 lugares
PRECIO HABITACIÓN PRIVADA: $11,354/persona · Anticipo $3,500 · 5 lugares
IMPORTANTE: Este paquete NO tiene tour de bioluminiscencia. Si alguien pregunta por bioluminiscencia, indica que ese tour es solo del paquete Año Nuevo al Desnudo.

AÑO NUEVO AL DESNUDO — DATOS DEFINITIVOS:
IMPORTANTE: Para el paquete Año Nuevo al Desnudo, las habitaciones (individual y doble) son en Hotel Juquila — NO en Hotel Paraíso. Hotel Paraíso es SOLO para Lunas de Octubre.
...
- Tarjeta de crédito con financiamiento: aplica cargo según tabla de arriba. Se procesa con Clip.
...
- NUNCA digas "el botón de abajo" — los botones aparecen automáticamente en el chat
- NUNCA digas "el sistema te guiará" — tú eres el sistema
...
• LÍMITE DE LONGITUD: Tus respuestas no deben superar los 800 caracteres. Si la información es extensa, prioriza lo más importante y termina invitando al cliente a preguntar por más detalles.
...
4. CERRAR → "¿Te aparto el lugar?" — si el usuario confirma, di "¡Perfecto! En un momento te muestro las opciones de pago" — los botones aparecen automáticamente
5. COBRAR → NUNCA pidas nombre, email ni datos bancarios — los datos se recopilan automáticamente después de que elijan cómo pagar
...
- PROMOCIÓN GLAMPING: Por tiempo limitado y hasta agotar existencias, incluye tienda de acampar y colchón inflable en préstamo. Siempre menciona esto cuando hables del glamping — es un diferenciador importante.
- Hotel Juquila (habitaciones): rústico, a calle y media de la playa, sin asignación de compañero

DATOS BANCARIOS PARA TRANSFERENCIA:
Banco: BBVA
CLABE: 012180015346960794
Beneficiario: Zipolite al Desnudo
Referencia: nombre del viajero

Cuando alguien pregunte por los datos bancarios, los datos de transferencia, o cómo pagar por transferencia, SIEMPRE incluye estos datos completos en tu respuesta. Nunca digas "te los mando por email" ni "los recibirás después". Los datos son públicos y deben darse de inmediato.
...
REGLA CRÍTICA: Nunca menciones bioluminiscencia cuando hables de Lunas de Octubre. Nunca menciones Hotel Juquila para Lunas de Octubre — ese hotel es del paquete Año Nuevo. Para Lunas de Octubre el hotel es SIEMPRE Hotel Paraíso.`;
...
function sanitizeResponse(text, pkgNombre = '') {
  // No reemplazar si el contexto O el texto mismo habla de Año Nuevo
  const esAnoNuevo = /año nuevo|ano nuevo/i.test(pkgNombre) ||
                     /año nuevo|ano nuevo/i.test(text);

  if (!esAnoNuevo) {
    text = text.replace(/Hotel Juquila/gi, 'Hotel Paraíso');
    text = text.replace(/Juquila/gi, 'Paraíso');
  }

  const lines = text.split('\n');
  const sanitized = lines.map(line => {
    if (line.toLowerCase().includes('bioluminiscencia') &&
        !line.toLowerCase().includes('año nuevo') &&
        !line.toLowerCase().includes('carrizalillo')) {
      return line.replace(/bioluminiscencia[^,.\n]*/gi, 'la magia de Zipolite');
    }
    return line;
  });
...
export async function handleWithClaude(chatId, userMessage, conv, contacto, paquetes, resenas, token, nombre, inlineButtons = null) {
  if (SALUDOS.includes(userMessage.toLowerCase()) && conv.estado_reserva?.step) {
    await supabase
      .from('conversaciones_telegram')
      .update({ estado_reserva: null })
      .eq('chat_id', chatId);
    conv = { ...conv, estado_reserva: null };
  }
  ...
  if (process.env.ANTHROPIC_API_KEY) {
    const resolvedNombre = conv.estado_reserva?.nombre || nombre;
    const contextExtra = resolvedNombre ? `El usuario se llama ${resolvedNombre}.` : '';
    ...
    // Truncar a máximo 900 caracteres para evitar que Telegram corte el mensaje
    if (reply && reply.length > 900) {
      const truncated = reply.substring(0, 900);
      const lastNewline = truncated.lastIndexOf('\n');
      reply = lastNewline > 500 ? truncated.substring(0, lastNewline) + '\n\n¿Quieres más detalles sobre alguna opción?' : truncated + '...';
    }
    reply = sanitizeResponse(reply, conv.pkg_context?.paquete_nombre || '');
  }
  ...
  if (agendar) {
    ...
    // Closing intent → show anticipo/total buttons directly, skip Claude's text
    const closingPhrasesReply = ['¿te aparto', '¿listo', '¿cómo pago'];
    const userConfirmPhrases = ['sí', 'si', 'dale', 'va', 'cómo pago', 'como pago'];
    const isClosingReply = closingPhrasesReply.some(ph => reply.toLowerCase().includes(ph));
    const isUserConfirm = userConfirmPhrases.some(ph => {
      const t = userMessage.toLowerCase().trim();
      return t === ph || t === ph + '!' || t.startsWith(ph + ' ');
    });
    const ctx = extractedCtx
      ? { ...extractedCtx, personas: personasDetected || conv.pkg_context?.personas || 1 }
      : conv.pkg_context || {};

    if ((isClosingReply || isUserConfirm) && ctx.precio && ctx.anticipo && !conv.estado_reserva?.step) {
      const personasNum = ctx.personas || 1;
      const anticipoTotal = (ctx.anticipo || 0) * personasNum;
      const totalCompleto = (ctx.precio || 0) * personasNum;

      await supabase.from('conversaciones_telegram')
        .update({ estado_reserva: { step: 'pidiendo_tipo_pago', ...ctx } })
        .eq('chat_id', chatId);

      await sendMessage(token, chatId,
        `¿Cómo quieres manejar el pago?`,
        [
          [{ text: `💰 Solo anticipo — $${anticipoTotal.toLocaleString('es-MX')}`, callback_data: 'tipo_anticipo' }],
          [{ text: `✅ Pago completo — $${totalCompleto.toLocaleString('es-MX')}`, callback_data: 'tipo_total' }]
        ]
      );
    ...
    // Fallback: show reservar_ button when closing phrase fires but ctx incomplete
    ...
    if (isClosingButton && !inlineButtons && paquetes.length > 0 && historial.length >= 6) {
