...
export async function generarContrato(reservacion_id) {
  try {
    ...
    const fileName = `${reservacion_id}_${Date.now()}.pdf`;
...
function buildPDF(reserva, viajeros, servicios, fechaInicioPaquete, fechaFinPaquete, total, saldo, duracion, totalPagado, metodoPago) {
  return new Promise((resolve, reject) => {
    ...
    const capitalize = s => s
      ...
      : '';
    const limpiarTexto = str => (str || '').replace(/[^\x00-\x7FÀ-ɏ -ÿ]/g, '').trim();
    ...
    doc.moveDown(0.5);
    doc.text(`Servicios incluidos: ${limpiarTexto(servicios)}`);
    doc.moveDown(1);
    ...
    doc.moveDown(0.5);
    doc.fontSize(10).font('Helvetica').fillColor('#111');
    doc.text(`Precio total del paquete: ${fmt(total)}`);
    doc.moveDown(0.3);
    doc.text(`Método de pago: ${fmtMetodo(metodoPago)}`);
    doc.moveDown(0.3);
    if (saldo === 0) {
      doc.font('Helvetica-Bold').fillColor('#1a9fa0').text(`PAGADO EN SU TOTALIDAD: ${fmt(totalPagado)}`);
      doc.font('Helvetica').fillColor('#111');
    } else {
      doc.text(`Anticipo pagado: ${fmt(totalPagado)}`);
      doc.moveDown(0.3);
      doc.text(`Saldo pendiente: ${fmt(saldo)}`);
    }
    doc.moveDown(1);
