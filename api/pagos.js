...
export default async function handler(req, res) {
  ...
  if (method === 'GET' && params.get('email') && params.get('cliente') === '1') {
    ...
    const { data: reservaciones, error: rErr } = await sb
      .from('reservaciones')
      .select('id, paquete_nombre, personas, total, estado, metodo_pago, fecha_inicio, fecha_fin, created_at, contrato_url, whatsapp, variantes_paquete(nombre), paquetes(foto_url)')
      .eq('email', email)
